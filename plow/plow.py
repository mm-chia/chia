...
#DESTS = ["/mnt/chia/8DJZG9XY", "/mnt/chia/8DK0AETH", "/mnt/chia/8DJZW07Y", "/mnt/chia/8DJZW1YY"]
...
async def plotfinder(paths, plot_queue, loop):
    for path in paths:
        for plot in Path(path).glob(f"**/*{EXTENSION}"):
...
async def plotwatcher(paths, plot_queue, loop):
    ...
    while True:
        ...
        if event.name.endswith(EXTENSION):
